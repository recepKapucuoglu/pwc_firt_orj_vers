               ...
               <table border="0" cellpadding="0" cellspacing="0" style="background-color: rgb(255, 255, 255); border: 1px solid rgb(222, 222, 222);" width="100%" class="oneColumn">
                  <tbody><tr>
                     <td align="center" valign="top" style="padding-bottom: 40px;" class="imgPost">
                        ...
                        <a href="#" target="_blank" style="text-decoration:none;" class="">
                           <img src="https://www.okul.pwc.com.tr/platform/images/banner-mailing.jpg" width="600" alt="" border="0" style="width:100%; max-width:600px; height:auto; display:block;" class="">
                        </a>
                     </td>
                  </tr>
                  ...
                  <tr>
                     <td align="center" valign="top" style="padding-bottom: 20px; padding-left: 20px; padding-right: 20px;" class="title">
                        ...
                        21 Aralık 2020 tarihinden itibaren aşağıda yer alan «Workshoplara Katıl» butonuna tıklayarak, e-posta adresinizle kayıt yaptırdığınız ve programdaki diğer Workshop’lara erişim linkine ulaşabilirsiniz. <br><br>      
                        Etkinliğimiz Global Meet platformu üzerinden gerçekleştirileceği için her Workshop’a katılırken ad soyad, e-posta adresi, şirket adı ve unvanınızı içeren kısa formu doldurmanızı rica ederiz.
...
if($safePost["selectedVideos"] == "not" OR empty($safePost["selectedVideos"])){
    $form_empty = true;
    $message_error .= errorMessage("<a href='' class='workshop'>En az bir workshop seçmelisiniz</a>");
}
// repeating ids
$repeating = explode(",", $safePost["selectedVideos"]);
$repeating = array_map('trim', $repeating);
$repeating = array_map('intval', $repeating);
if(in_array(3, $repeating) && in_array(4, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(5, $repeating) && in_array(40, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(41, $repeating) && in_array(42, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(44, $repeating) && in_array(45, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(47, $repeating) && in_array(48, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(49, $repeating) && in_array(50, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(51, $repeating) && in_array(52, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(55, $repeating) && in_array(56, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(60, $repeating) && in_array(61, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(62, $repeating) && in_array(63, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(65, $repeating) && in_array(66, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
else if(in_array(68, $repeating) && in_array(69, $repeating)) { $form_empty = true; $message_error .= errorMessage("Aynı gün ve saatteki workshoplardan sadece birisini seçebilirsiniz."); }
...
// Days array
$days = array(
    "monday" => "21 Aralık Pazartesi",
    "tuesday" => "22 Aralık Salı",
    "wednesday" => "23 Aralık Çarşamba",
    "thursday" => "24 Aralık Perşembe",
    "friday" => "25 Aralık Cuma"
);
...
foreach ($arrayofIds as $selectedId){
    ...
    try {
        ...
        $params = array($safePost["adsoyad"], $safePost["firma_adi"], $safePost["email"], $safePost["unvan"], $safePost["telefon"],
            $safePost["gsm"], $aydinlatma_metni, $course["name"], $days[$course["day"]], $course["start_time"], $course["end_time"],
            $dateNow, $safePost["iletisimizni"], $alumni,$firma_unvani,$vergi_dairesi,$vergi_no,$adres,$fatura_tipi,$_SERVER['REMOTE_ADDR']);
        $sql = $pdo->prepare("INSERT INTO event_signup(isim, firma, sirketmail, unvan, telefon, ceptelefon, aydinlatmametni,
    sunumismi, sunumgunu, sunumbaslangicsaati, sunumbitissaati, kayitzamani, iletisimizni, alumni,fatura_adi,vergi_dairesi,vergi_no,adres,fatura_tipi,ipaddress) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $sql->execute($params);
        $count = $sql->rowCount();
        ...
        $googleCalendar = generateGoogle("PwC Türkiye 19. Çözüm Ortaklığı Platformu | {$course["name"]}",
            $startIso, $endIso, "Etkinliğe kayıt olduğunuz mail adresi ile giriş yapabilirsiniz.", "https://www.okul.pwc.com.tr/platform/giris");
        $googleCalendar = "<a href='$googleCalendar'><img width='30px' height='20px' src='https://www.okul.pwc.com.tr/platform/images/takvim-icon-red.png' /></a>";
        $mailTemplate .= <<<EOT
                        <tr style="border: 2px solid black;">
                        <td class="tg-cly1" style="padding: 10px;">{$course["name"]}</td>
                        <td class="tg-nrix" style="padding: 10px;">{$days[$course["day"]]}</td>
...
$mailTemplate .= <<<EOT
                ...
                <td align="center" valign="top" style="padding-bottom: 20px; padding-left: 20px; padding-right: 20px;" class="title">
                    <!-- Main Title Text // -->
                    <h2 class="bigTitle" style="color: #d93954; font-family:"Open Sans", Helvetica, Arial, sans-serif; font-size:26px; font-weight:600; font-style:normal; letter-spacing:normal; line-height:34px; text-align:center; padding:0; margin:0;">
                        Workshoplara Ulaşım
   
                    </h2>
                    21 Aralık’tan itibaren aşağıdaki butona tıklayarak, e-mail adresinizle kayıt yaptırdığınız ve
                    programdaki tüm workshop’lara erişim linkine ulaşabilirsiniz.
                    <br><br><a href="https://www.okul.pwc.com.tr/platform/giris"><img src="https://www.okul.pwc.com.tr/platform/images/mail-katil-buton.jpg"></a><br><br>
                    Etkinliğimiz Global Meet platformu üzerinden gerçekleştirileceği için her
                    workshop’a katılırken Ad Soyad, E-posta adresiniz, şirket adı ve
                    görevinize ilişkin kısa formu doldurmanızı rica ederiz.
                    </td>
                    </tr>
                     </td>
                  </tr>
                  ...
                  <tr>
                     <td align="center" valign="top" style="padding: 10px;" class="brandInfo">
                        ...
                        <p class="smlText" style="color:#313131; font-family:"Open Sans", Helvetica, Arial, sans-serif; font-size:11px; font-weight:400; line-height:18px; text-align:center; margin:0; padding:0;">
                           © 2020 PwC Türkiye. Tüm hakları saklıdır. Bu belgede PwC ifadesi, PwC ağını veya PwC ağının üyesi olan bağımsız ve farklı tüzel kişiliklerden oluşan PwC Türkiye\'yi ifade etmektedir.<br>
...
$mail->Subject = '19. Çözüm Ortaklığı Platformu | Kaydınız Tamamlanmıştır';
$mailpwc->Subject = '19. Çözüm Ortaklığı Platformu | Yeni Kayıt';
