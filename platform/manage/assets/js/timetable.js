...
function getPosition(axis, start_time) {
	return parseInt((calHour(start_time) - axis) * 110, 10); // 1 cell top = 55px;
}

function getHeight(start_time, end_time) {
	return parseInt((calHour(end_time) - calHour(start_time)) * 120, 10); // 1 cell = 55px;
}
	...
	timetableString += '<div class="timetable-week ' + show_time + '">';
		...
		timetableString += '<div class="timetable-columns">';
			...
			for (var m = 0; m < wordDay.length; m++) {
				...
				month = d.getMonth() + 1;
				year = d.getFullYear();
				header_time = (el.attr('data-header-time') == 'hide') ? '' : '<br><span>' + april + ' ' + 'Aralık' + ', ' + year + '</span>';
				april++
				...
				timetableString += '<div class="timetable-column">';
					// Header
					if (screen.width > 768) {
						timetableString += '<div class="timetable-column-header ' + last + '">Salı' + header_time + '</div>';
					} else {
						timetableString += '<div class="timetable-column-header ' + last + '">Salı' + header_time + '</div>';
					}
						...
						for (var t = 0; t < timetables.length; t++) {
							if (timetables[t].start_time && timetables[t].end_time) {
								...
								timetableString += 	'<div class="timetable-item">'
														+ '<a id="a-' + timetables[t].pid + '" class="timetable-title color-' + timetables[t].color + '" style="top:' + position + 'px; height:' + height + 'px; ' + item_width + item_left + '" href="#timetable-popup-' + timetables[t].id + '" class="open-popup-link">'
														...
														+ '<div id="timetable-popup-' + timetables[t].id + '" class="timetable-popup zoom-anim-dialog mfp-hide">'
															...
															+ '<div class="popup-body">'
																...
																+ '<div class="timetable-desc"><a class="btn" style="font-size: 16px;font-weight: 700;display: block;color: #fff;text-transform: uppercase;background: #e0301e;height: 50px;padding: 0 35px;line-height: 50px;border-radius: 3px;-webkit-border-radius: 3px;-ms-border-radius: 3px;-o-transition: all 0.4s ease;transition: all 0.4s ease;-webkit-transition: all 0.4s ease;-moz-transition: all 0.4s ease;-ms-transition: all 0.4s ease;outline: none;text-decoration: none;cursor: pointer;" target="_blank" href="' + timetables[t].description + '">Oturuma Katıl</a></div>'
															+ '</div>'
														+ '</div>'
													+ '</div>';
							}
